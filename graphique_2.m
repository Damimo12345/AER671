...
Range = 150:10:1500;        % [150 200 250 … 1500]
N      = numel(Range);      % nombre d'éléments

% 3) bouclez sur l'indice i
for i = 1:N

    % À CHANGER À CHANQUE ITÉRATION (faire une boucle)
    Range_start= 0; %nm commencer à 600nm
    Range_iteration = Range(i) ;
    
    while (abs(Range_start-Range_iteration) > 0.01)
...
figure
plot(Range, mylist, '-o','LineWidth',1.5)
grid on
xlabel('Range (ft)')
ylabel('Takeoff Weight W_{TO} (lbf)')
title('W_{TO} vs Range')